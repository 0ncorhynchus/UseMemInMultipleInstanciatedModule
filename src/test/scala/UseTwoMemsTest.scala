...
class UseTwoMemsTest extends AnyFlatSpec with ChiselScalatestTester {
  val DATA_WIDTH = 16
  val ADDR_WIDTH = 4
  ...
  val memFile0 = File.createTempFile("UseTwoMemsTest", ".hex")
  val memFile1 = File.createTempFile("UseTwoMemsTest", ".hex")
  ...
  it should "work" in {
    test(new UseTwoMems(ADDR_WIDTH, DATA_WIDTH, memFile0.getPath(), memFile1.getPath())) { c =>
      c.io.en.poke(true.B)
      for (adr <- 0 until DEPTH) {
